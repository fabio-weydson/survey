...
.service('irkConsentDocument', function($filter) {
    ...
    service.initDocument = function() {
        pdfDefinition = { 
            ...
            footer: function(currentPage, pageCount) { return { text: 'Pagina ' + currentPage.toString() + ' de ' + pageCount, style: 'footer' }; },
        };
    };
    ...
    function($rootScope, $timeout, $interval, $compile, $ionicSlideBoxDelegate, $ionicHistory, $ionicScrollDelegate, $ionicNavBarDelegate, $ionicActionSheet, $ionicModal, $ionicPopup, $ionicPlatform, irkResults) {
        return {
            ...
            template:
                ...
                '<div>'+
                '<a class="button button-block button-outline button-positive irk-bottom-button irk-button-step-next" ng-click="doStepNext()" irk-step-next>Seguinte</a>'+
                '<a class="button button-block button-clear button-positive irk-bottom-button irk-button-step-skip" ng-click="doSkip()" irk-step-skip>Pular Pergunta</a>'+
                '</div>'+
                '</ion-footer-bar>'+
                //FOOTER BAR FOR CONSENT STEPS
                '<ion-footer-bar class="irk-bottom-bar irk-bottom-bar-consent" keyboard-attach irk-consent-bar>'+
                '<button class="button button-block button-outline button-positive irk-bottom-button irk-button-step-next" ng-click="doStepNext()" irk-step-next>Seguinte</button>'+
                '</ion-footer-bar>'+
                ...
                '<div class="buttons">'+
                '<button class="button button-clear button-positive irk-button-consent-disagree" ng-click="doDisagree()">Discordo</button>'+
                '</div>'+
                '<h1 class="title"></h1>'+
                '<div class="buttons">'+
                '<button class="button button-clear button-positive irk-button-consent-agree" ng-click="doAgree()">Concordo</button>'+
                '</div>'+
                '</ion-footer-bar>'+
                '</div>',
        ...
        link: function(scope, element, attrs, controller) {
            scope.$on("slideBox.slideChanged", function(e, index, count) {
                ...
                if (stepType!='IRK-VISUAL-CONSENT-STEP' && stepType!='IRK-CONSENT-SHARING-STEP' && stepType!='IRK-CONSENT-REVIEW-STEP')
                    element.text('Pergunta ' + (index+1) + ' de ' + count);
                else
                    element.text('');
            });
        }
        ...
        link: function(scope, element, attrs, controller) {
            scope.$on("slideBox.slideChanged", function(e, index, count) {
                ...
                if (stepType=='IRK-INSTRUCTION-STEP' && footerAttach) {
                    ...
                    element.text(step.attr('button-text') ? step.attr('button-text') : 'Iniciar');
                }
    ...
    return {
        ...
        template: function(elem, attr) {
            return 	'<div class="irk-offcentered-container"><div class="irk-offcentered-content">'+
                    ...
                    (attr.footerAttach && attr.footerAttach=='true'?'':'<button class="button button-outline button-positive irk-button-instruction" ng-click="$parent.doNext()">'+(attr.buttonText ? attr.buttonText : 'Iniciar')+'</button>')+
                    '</div></div>'
        },
        link: function(scope, element, attrs, controller) {
